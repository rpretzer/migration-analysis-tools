# ViewModels wave 2: batch 4

**Title:** ViewModels wave 2: batch 4

**Story:**
As a user of the app, I want the fourth batch of screens migrated to ViewModels so that those screens survive rotation without data loss and the codebase continues toward full MVVM coverage.

**Context / Background:**
This is the fourth of six batches in the wave-2 ViewModel migration. The pattern was established in story B-3 and the batch plan was defined in story B-7. The same structure and acceptance criteria as previous batches apply here.

**In scope:**
- [ ] Introduce ViewModels for all screens assigned to batch 4 (screen list defined in story B-7)
...
```
Given all screens in batch 4 have been migrated
When each screen is rotated
Then no data loss occurs on any screen
```

```
Given batch 4 migration is complete
When a code search is run for onActive and onInactive
Then zero results remain for any screen in batch 4
```

```
Given batch 4 migration is complete
When a code search is run for the Controller classes corresponding to batch 4 screens
Then those classes do not exist; they have been deleted
```

```
Given each screen in batch 4 is exercised
...
- Story B-10 (batch 3) should be stable before batch 4 begins.
